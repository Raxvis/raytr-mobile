...
const ItemPage = () => {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', async () => {
      const item = await getItemWithRatings(itemId, categoryId);
    ...
    <>
      <Stack.Screen
        options={{
          ...
          headerRight: () => (
            <NavButton
              onPress={() => router.navigate(`/category/${categoryId}/item/${itemId}/edit`)}
              Icon={Cog6ToothIcon}
              color="white"
            />
          ),
      ...
      <FlatList
        ...
        renderItem={({ item: rating }) => <RatingItem categoryId={categoryId} item={item} rating={rating} />}
      />
    </>
  );
};
